...
const game = {
  ...
  backGrounds: [], //背景の配列
  image: [],
  bgm1: new Audio('chrome-dino-game-images/bgm/fieldSong.mp3'),
  bgm2: new Audio('chrome-dino-game-images/bgm/jump.mp3'),
  score: 0,
  state: 'loading',
  timer: null,
};

//複数画像読み込み 画像読み込んだら初期化(init)
let imageLoadCounter = 0;
for(const imageName of imageNames){
  const imagePath = `chrome-dino-game-images/image/${imageName}.png`;
...
//30ミリ秒の間隔で呼び出されるticker関数
function ticker(){
  //画面クリア
  ctx.clearRect(0, 0, canvas.width, canvas.height);

  //背景の作成 createBackGround関数を作って呼び出す
